...
- see [https://github.com/transifex/cli](https://github.com/transifex/cli) for instructions
...
- `make po` / `make mo` — English and German (`en`, `de`) only; typical local workflow and Transifex source push.
- `make po_all` / `make mo_all` — all locales (e.g. after `tx pull -af`).
...
`make po` (English and German) or `make po_all` (all locales).
...
`make po` will also fill missing English `msgstr` from the corresponding `msgid` (`msgen`), meaning
...
Now, `make po` / `make po_all` creates message files in the locale-fork folder and
...
2. By using the context of the translations. Django docs on contextualizing translations
  - [in python code]]([https://docs.djangoproject.com/en/3.2/topics/i18n/translation/#contextual-markers](https://docs.djangoproject.com/en/3.2/topics/i18n/translation/#contextual-markers))
