...
def test_get_change_perm(rf, topic_factory):
    ...
    assert 'a4_candy_topicprio.view_topic' == context['view_perm']
    assert rules.perm_exists(context['view_perm'])
    assert 'a4_candy_topicprio.add_topic' == context['add_perm']
    assert rules.perm_exists(context['add_perm'])
    assert 'a4_candy_topicprio.change_topic' == context['change_perm']
    assert rules.perm_exists(context['change_perm'])
    assert 'a4_candy_topicprio.delete_topic' == context['delete_perm']
