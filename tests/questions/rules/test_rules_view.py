...
perm_name = 'a4_candy_questions.view_question'
...
def test_pre_phase(phase_factory, question_factory, user):
...
@pytest.mark.django_db
def test_phase_active(phase_factory, question_factory, user):
    phase, _, project, item = setup_phase(phase_factory, question_factory,
                                          phases.IssuePhase)
    anonymous, moderator, initiator = setup_users(project)
...
@pytest.mark.django_db
def test_phase_active_project_private(phase_factory, question_factory,
                                      user, user2):
    phase, _, project, item = setup_phase(phase_factory, question_factory,
                                          phases.IssuePhase,
