...
def test_upload_image_validation(organisation, project_factory,
                                 image_factory):
    ...
    for choice in SOCIAL_MEDIA_CHOICES:
        ...
        invalid_image = image_factory(int(min_width * 0.8),
                                      int(min_height * 0.8))
        ...
        with translation.override('en_GB'):
            assert 'Image must be at least {} pixels wide'.format(min_width) \
                   in content_form.errors['image']
            assert 'Image must be at least {} pixels high'.format(min_height) \
                   in content_form.errors['image']
...
def test_content_create_initial_data(organisation, project_factory):
    ...
    description_max_length = \
        SOCIAL_MEDIA_SIZES[format]['description_max_length']

    content_form = CommunicationContentCreationForm(
        project=project, format=format)
    assert content_form.fields['title'].initial == \
           project.name[:title_max_length]
    assert content_form.fields['description'].initial == \
