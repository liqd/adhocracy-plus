...
def test_project_choice_view(client, user, organisation, project_factory):
    ...
    url = reverse('a4dashboard:communication-content',
...
@pytest.mark.django_db
def test_content_create_view(client, user, organisation, project_factory,
                             image_factory):
    initiator = organisation.initiators.first()
    project = project_factory(organisation=organisation)
    format = SOCIAL_MEDIA_CHOICES[0][0]

    url = reverse('a4dashboard:communication-content-create',
                  kwargs={'organisation_slug': organisation.slug,
...
@pytest.mark.django_db
def test_valid_data_sharepic_created(client, organisation, project_factory,
                                     image_factory):
    initiator = organisation.initiators.first()
    ...
    for choice in SOCIAL_MEDIA_CHOICES:
        ...
        data['image'] = image_factory(sizes['img_min_width'],
                                      sizes['img_min_height'])
        url = reverse('a4dashboard:communication-content-create',
                      kwargs={'organisation_slug': organisation.slug,
                              'project_slug': project.slug,
                              'format': format})
        ...
        preview_image = Image.open(
            BytesIO(base64.b64decode(preview_image_data))
        )
        assert preview_image.size == (sizes['img_min_width'],
                                      sizes['overall_height'])


@pytest.mark.django_db
def test_invalid_data_no_sharepic_created(client, organisation,
                                          project_factory, image_factory):
    initiator = organisation.initiators.first()
    project = project_factory(organisation=organisation)
    format = SOCIAL_MEDIA_CHOICES[0][0]
