...
perm_name = "a4_candy_ideas.view_idea"
...
def test_phase_active_project_private(
    phase_factory, idea_factory, user, user2, member_factory
):
    phase, _, project, item = setup_phase(
        phase_factory,
        idea_factory,
...
@pytest.mark.django_db
def test_phase_active_project_draft(phase_factory, idea_factory, user, member_factory):
    phase, _, project, item = setup_phase(
        phase_factory, idea_factory, phases.CollectPhase, module__project__is_draft=True
    )
    anonymous, moderator, initiator = setup_users(project)
    member = member_factory(organisation=project.organisation)
...
@pytest.mark.django_db
def test_post_phase_project_archived(phase_factory, idea_factory, user, member_factory):
    phase, _, project, item = setup_phase(
        phase_factory,
        idea_factory,
        phases.CollectPhase,
