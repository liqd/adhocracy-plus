...
def test_detail_view(
    client, user, phase_factory, live_question_factory, interactive_extra_fields_factory
):
    ...
    with freeze_time(parse("2013-01-01 18:00:00 UTC")):
        ...
        assert_template_response(
            response, "a4_candy_interactive_events/livequestion_module_detail.html"
        )
        assert "Live now" not in response.content.decode()

    interactive_extra_fields = interactive_extra_fields_factory(
        module=module, creator_id=user.id
    )
