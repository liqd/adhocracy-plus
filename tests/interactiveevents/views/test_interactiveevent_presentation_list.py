...
def test_presentation_list_view(
    client, user, phase_factory, like_factory, live_question_factory
):
    ...
    assert Like.objects.all().count() == len(livequestion.livequestion_likes.all())
    ...
    with freeze_phase(phase):
        ...
        assert livequestion_2 not in response.context_data["livequestion_list"]
