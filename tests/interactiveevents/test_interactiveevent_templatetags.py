...
def test_react_questions(rf, user, live_question):
    ...
    template = (
        "{% load react_interactiveevents %}" "{% react_interactiveevents module %}"
    )
    context = {"request": request, "module": module}

    questions_api_url = reverse(
        "interactiveevents-list", kwargs={"module_pk": module.pk}
    )
    present_url = reverse(
        "question-present",
        kwargs={
