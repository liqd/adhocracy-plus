...
def test_anonymous_can_view_and_cannot_change_questions(
    ...
    url = reverse("interactiveevents-list", kwargs={"module_pk": module.pk})
    url_detail = reverse(
        "interactiveevents-detail",
        kwargs={"module_pk": module.pk, "pk": livequestion.pk},
    )
...
@pytest.mark.django_db
def test_anonymous_can_add_like_during_active_phase(
    apiclient, phase_factory, live_question_factory
):
...
@pytest.mark.django_db
def test_user_cannot_add_like_during_non_active_phase(
    apiclient, phase_factory, live_question_factory, user
):
