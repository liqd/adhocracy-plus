...
def test_extrafields_dashboard_view(client, phase_factory):
    ...
    labels = [
        entry["label"]
        for entry in response.context_data["dashboard_menu"]["modules"][0]["menu"]
    ]
    assert "Media" in labels
    assert_template_response(
        response, "a4_candy_interactive_events/extrafields_dashboard_form.html"
    )
    assert "live_stream" in response.context_data["form"].fields
    assert "event_image" in response.context_data["form"].fields

    data = {
        "live_stream": "<div>Livestram data</div>",
        # that doesnt work
    ...
    assert (
        response.context_data["extrafieldsinteractiveevent"].live_stream
        == data["live_stream"]
    )
