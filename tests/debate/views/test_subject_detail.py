...
def test_detail_view(client, phase_factory, subject_factory):
    ...
    with freeze_phase(phase):
        ...
        assert_template_response(response, "a4_candy_debate/subject_detail.html")
        assert response.status_code == 200


@pytest.mark.django_db
def test_detail_view_private_not_visible_anonymous(
    client, phase_factory, subject_factory
):
    phase, module, project, subject = setup_phase(
        phase_factory, subject_factory, phases.DebatePhase
    )
...
def test_detail_view_semipublic_participation_only_participant(
    ...
    api_url = reverse(
        "comments-list", kwargs={"content_type": subject_ct.pk, "object_pk": subject.pk}
    )
