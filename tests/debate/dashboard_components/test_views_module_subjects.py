...
def test_edit_view(client, phase_factory, subject_factory):
    ...
    assert_template_response(response, "a4_candy_debate/subject_dashboard_list.html")


@pytest.mark.django_db
def test_subject_create_view(client, phase_factory, category_factory):
    phase, module, project, item = setup_phase(phase_factory, None, DebatePhase)
    initiator = module.project.organisation.initiators.first()
    url = reverse(
...
@pytest.mark.django_db
def test_subject_update_view(client, phase_factory, subject_factory, category_factory):
    phase, module, project, item = setup_phase(
        phase_factory, subject_factory, DebatePhase
    )
    initiator = module.project.organisation.initiators.first()
