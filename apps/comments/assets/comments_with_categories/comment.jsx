...
import ratings from 'adhocracy4'
...
const { ratings: { RatingBox } } = ratings
...
export default class Comment extends React.Component {
  ...
  renderComment () {
    ...
    if (this.state.edit) {
      ...
      if (this.props.children.length > 400 && this.state.shorten) {
        comment = <div className={'comment-text' + (this.state.anchored ? ' comment-text--highlighted' : '')} dangerouslySetInnerHTML={safeHtml(this.props.children.substring(0, 400) + '...')} />
      } else {
        comment = <div className={'comment-text' + (this.state.anchored ? ' comment-text--highlighted' : '')} dangerouslySetInnerHTML={safeHtml(this.props.children)} />
      }
    }
    return comment
  }
  ...
  renderCategories () {
    if (!this.state.edit && this.displayCategories()) {
      ...
      var categoryHtml = Object.keys(categories).map(function (objectKey, index) {
        ...
        categoryClassName = 'badge comment-badge comment-badge__' + objectKey
  ...
  render () {
    ...
    return (
      <div>
        <div className={(this.isOwner() ? 'comment comment-owner' : 'comment')}>
          <a className="comment-anchor" id={`comment_${this.props.id}`} href={`./?comment=${this.props.id}`}>{`Comment ${this.props.id}`}</a>
          <div className="container">
            ...
            <div className="comment-box row position-relative">
              {(this.props.is_moderator_marked && this.displayCategories()) &&
                <span className={this.context.isAuthenticated && !this.props.is_deleted && (this.isOwner() || this.context.isModerator) ? 'comment--featured--menu position-absolute' : 'comment--featured position-absolute'}>
                  <img src="/static/images/badge_featured.png" alt="Featured comment" />
                </span>}
              <div className="comment-box_left pr-0 pr-md-3">
                {userImage}
                <h5 className={this.props.is_deleted ? 'comment-deleted-author' : 'comment-author'}>
                  {this.props.is_deleted ? this.props.user_name
                    : <a href={userProfile} className="u-color-primary">{this.props.user_name}</a>}
                </h5>
                <div className="comment-moderator">{moderatorLabel}</div>
                <div className="comment-submission-date">{lastDate}</div>
              </div>
              <div className={this.state.anchored ? 'comment-box_right comment-border_red' : 'comment-box_right'}>
                ...
                {this.context.isAuthenticated && !this.props.is_deleted && (this.isOwner() || this.context.isModerator) &&
                  ...
                  />}
                {this.renderComment()}
                {this.props.children.length > 400 && this.state.shorten && <button className="btn btn--sm btn--link-light pl-0" onClick={this.showMore.bind(this)}>{django.gettext('Read more...')}</button>}
                {this.props.children.length > 400 && !this.state.shorten && <button className="btn btn--sm btn--link-light pl-0" onClick={this.showLess.bind(this)}>{django.gettext('Read less')}</button>}
                <div className="action-bar">
                  <nav className="navbar navbar-default navbar-static comment-navbar">
                    ...
                    <div className="comment-nav">
                      {this.props.child_comments && this.props.child_comments.length > 0 &&
                        <button className="btn comment-reply-button" type="button" onClick={this.toggleShowComments.bind(this)}>
                          <i className={this.state.showChildComments ? 'fa fa-minus' : 'icon-comment'} aria-hidden="true" />
                      ...
                      {!this.props.is_deleted &&
                        <a
                          className="btn comment-report-link" href={`?comment_${this.props.id}`}
                          data-toggle="modal" data-target={`#share_comment_${this.props.id}`}
                        >{django.gettext('Share')}
                        </a>}
                      {!this.props.is_deleted && !this.isOwner() &&
                        <a
                          className="btn comment-report-link" href={`#report_comment_${this.props.id}`}
                          data-toggle="modal"
            ...
            {this.state.displayNotification &&
              <div className="comment-success-notification"><i className="icon-check" /> {successMessage}</div>}
          </div>
        </div>
        <div className="container">
