...
export default class CommentBox extends React.Component {
  ...
  render () {
    return (
      <div>
        <div className={this.state.displayForm ? 'commentbox_form container' : 'comment-form-hidden'}>
          <CommentForm
        ...
        <div className={(this.state.comments.length === 0 && this.state.loading) ? 'hidden' : 'comments_nav_parent'}>
          <nav className="bg-faded comments_nav">
            <div className="container">
              <div className="row px-sm-3">

                <div className={this.state.search !== '' ? 'comments_nav__text' : 'hidden'}>
                  <span className="comments_nav__span">{this.state.commentCount + ' ' + django.ngettext('entry found for ', 'entries found for ', this.state.commentCount)}</span>

                  <button className="btn btn--sm comments_nav__search-result" type="button" onClick={this.handleClickResult}>{this.state.search}<i className="icon-close" aria-label={django.gettext('Clear search')} /></button>
                </div>
                ...
                <div className="comments_nav__search offset-lg-1 mt-2 mt-sm-0 pr-1 pl-md-3 mx-3 mx-sm-0">
                  <input type="search" id="search-input" onKeyPress={this.handleEnterSearch} placeholder={django.gettext('SEARCH CONTRIBUTIONS')} className="comments_nav__search-input" />
                  <button className="comments_nav__search-btn btn btn--no-padding btn--no-border" type="button" onClick={this.handleClickSearch}><i className="icon-search comments_nav__search-icon" aria-label={django.gettext('Search contributions')} /></button>
                </div>

                <div className="comments_nav__dropdown pr-sm-0">
                  <div className="dropdown">
                    <button
                      className="btn btn--transparent dropdown-toggle btn--border mt-2 mt-sm-0 comments_nav__btn" type="button"
                      id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
                    >
                      <span className={this.state.filter === 'all' ? 'comments_nav__btn-text btn--upper' : 'hidden'}>{django.gettext('display: ')}{this.state.filterDisplay}</span>
                      <span className={this.state.filter !== 'all' ? 'comments_nav__btn-text small-screen btn--capitalize' : 'hidden'}>{this.state.filterDisplay}</span>

                      <i className={this.state.filter === 'all' ? 'icon-arrow_dropdown comments_nav__btn-icon ' : 'icon-check comments_nav__btn-icon'} aria-hidden="true" />
                    </button>
                ...
                <div className="comments_nav__dropdown pr-sm-0">
                  <div className="dropdown">
                    <button
                      ...
                      id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
                    >
                      <span className={this.state.sort === 'mom' ? 'comments_nav__btn-text btn--upper' : 'hidden'}>{django.gettext('sorted by: ')}{sorts[this.state.sort]}</span>
                      <span className={this.state.sort !== 'mom' ? 'comments_nav__btn-text small-screen btn--capitalize' : 'hidden'}>{sorts[this.state.sort]}</span>
                      <i className={this.state.sort === 'mom' ? 'icon-arrow_dropdown comments_nav__btn-icon' : 'icon-check comments_nav__btn-icon'} aria-hidden="true" />
                    </button>
                    <div className="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
          ...
          <div className={this.state.loadingFilter ? 'row justify-content-center loading loading-nav' : 'hidden'}>
        ...
        <div className={this.state.loading ? 'row justify-content-center loading' : 'hidden'}>
          <i className="fa fa-spinner fa-pulse" />
        </div>
      </div>
    )
