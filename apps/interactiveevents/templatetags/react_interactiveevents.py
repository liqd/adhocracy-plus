...
def react_interactiveevents(context, obj):
    ...
    is_moderator = user.has_perm(
        "a4_candy_interactive_events.moderate_livequestions", obj
    )
    categories = [category.name for category in obj.category_set.all()]
    category_dict = {category.pk: category.name for category in obj.category_set.all()}
    questions_api_url = reverse("interactiveevents-list", kwargs={"module_pk": obj.pk})

    private_policy_label = str(
        _(
    ...
    data_protection_policy_url = get_important_page_url("data_protection_policy")

    likes_api_url = "/api/livequestions/LIVEQUESTIONID/likes/"
    present_url = reverse(
        "question-present",
        kwargs={
            "module_slug": obj.slug,
    ...
    would_have_liking_permission = NormalUser().would_have_perm(like_permission, obj)
    ...
    attributes = {
        ...
        "hasLikingPermission": (has_liking_permission or would_have_liking_permission),
        "hasAskQuestionsPermission": (
            has_ask_questions_permissions or would_have_ask_questions_permission
        ),
    ...
    return format_html(
        '<div data-aplus-widget="questions" ' 'data-attributes="{attributes}"></div>',
...
def react_interactiveevents_present(context, obj):
    ...
    return format_html(
        '<div data-aplus-widget="present" ' 'data-attributes="{attributes}"></div>',
        attributes=json.dumps(attributes),
    )
