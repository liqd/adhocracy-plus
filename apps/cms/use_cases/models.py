...
class UseCaseIndexPage(Page):
    subtitle_de = models.CharField(max_length=250, blank=True, verbose_name="Title")
    subtitle_en = models.CharField(max_length=250, blank=True, verbose_name="Title")
        ...
        if category:
            ...
            for category_choice in CATEGORY_CHOICES:
                if category_choice[0] == category:
                    context["get_current_category_display"] = category_choice[1]
        return context
...
class UseCasePage(Page):
    ...
    title_de = models.CharField(max_length=250, blank=True, verbose_name="German Title")
    ...
    def get_context(self, request):
        ...
        if category:
            try:
                use_cases = UseCasePage.objects.filter(category=category).exclude(
                    id=self.id
                )
            except ValueError:
                use_cases = []
    ...
    en_content_panels = [FieldPanel("title_en"), FieldPanel("body_streamfield_en")]
