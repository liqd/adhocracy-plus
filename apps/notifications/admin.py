...
def notification_strategies_overview(request):
    ...
    for notification_type in NotificationType:
        ...
        if email_class and strategy_class:
            ...
            try:
                rendered = _render_email_template(email_class, strategy_class, template_name, request.user)
    ...
    return render(request, 'admin/notification_strategies_overview.html', {
...
def _get_strategy_for_type(notification_type):
    ...
    strategy_map = {
        ...
        NotificationType.USER_CONTENT_CREATED: getattr(strategies, 'UserContentCreated', None),
    }
...
def _render_email_template(email_class, strategy_class, template_name, current_user):
    ...
    try:
        ...
        mock_object = _create_mock_object(strategy_class.__name__, real_project, current_user)
        ...
        email_context = email_instance.get_context() if hasattr(email_instance, 'get_context') else {}
        
        # Merge contexts
        context = {**strategy_context, **email_context}
        context.update({
            'email': email_instance,
            'site': email_instance.get_site() if hasattr(email_instance, 'get_site') else 
                   type('MockSite', (), {'name': 'Test Site', 'domain': 'example.com'})(),
