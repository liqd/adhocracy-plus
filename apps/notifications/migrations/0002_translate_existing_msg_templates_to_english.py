...
def fix_existing_message_templates(apps, schema_editor):
    Notification = apps.get_model("a4_candy_notifications", "Notification")

    for notification in Notification.objects.all():
        current = notification.message_template
        if current in translation_map:
            notification.message_template = translation_map[current]
            notification.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(fix_existing_message_templates, reverse_fix),
    ]
