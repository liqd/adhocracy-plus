...
class PhaseStarted(ProjectNotificationStrategy):
    def get_in_app_recipients(self, phase) -> List[User]:
        return self._get_phase_recipients(
            phase, NotificationType.PHASE_STARTED, "in_app"
        )

    def get_email_recipients(self, phase) -> List[User]:
        return self._get_phase_recipients(
            phase, NotificationType.PHASE_STARTED, "email"
        )
    ...
    def create_notification_data(self, phase) -> dict:
        ...
        return {
            ...
            "context": {
                "phase": phase.name,
                "phase_url": (
                    phase.get_absolute_url()
                    if hasattr(phase, "get_absolute_url")
                    else project.get_absolute_url()
                ),
...
class PhaseEnded(ProjectNotificationStrategy):
    ...
    def create_notification_data(self, phase) -> dict:
        ...
        return {
            ...
            "message_template": _(
                "The phase '{phase}' in {project} has been completed"
            ),
            "context": {
                "phase": phase.name,
                "phase_url": (
